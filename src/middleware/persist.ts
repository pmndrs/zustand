...
type WithPersist<S, A> = S extends { getState: () => infer T }
  ? Write<S, StorePersist<Extract<T, UnknownState>, A>>
  : never
...
interface PersistOptions<T extends UnknownState, U> {
  ...
  ): MaybePromise<PersistentStorageValue<T>>
...
type PersistentStorageNameImpl = string & { __isPersistentStorageName: true }
...
const persistImpl: PersistImpl =
  (storeInitializer, _options) => (parentSet, parentGet, parentStore) => {
    ...
    const persistentStorageSetItem = (
      serializedState: Serialized<PersistentStorageValueImpl>
    ...
    const hydrate = () => {
      ...
      const postRehydrationCallback =
        options.onRehydrateStorage?.(parentGet()) || undefined

      return thenablify(persistentStorageGetItem)()
        .then((serialized) =>
          serialized !== null ? options.deserialize(serialized) : null
        )
        .then((storageValue) => {
        ...
        .then((migratedState) => {
          initialStateFromPersistentStorage = options.merge(
            migratedState!,
            initialState
          )
        ...
        .then(() => {
          postRehydrationCallback?.(
            initialStateFromPersistentStorage,
            undefined
          )
          hasHydrated = true
...
type EPersistDefaultedOptions = Required<
  Omit<PersistOptionsImpl, 'name' | 'onRehydrateStorage' | 'migrate'>
>

const defaultOptions: EPersistDefaultedOptions = {
  getStorage: (() =>
    localStorage) as unknown as EPersistDefaultedOptions['getStorage'],
  ...
  partialize: (<T>(x: T) =>
    x) as unknown as EPersistDefaultedOptions['partialize'],
...
export interface Thenable<T> {
  then: <U>(
    onFulfilled: (value: T) => U | Promise<U> | Thenable<U>
  ) => Thenable<U>
  catch: <U>(
    onRejected: (error: unknown) => U | Promise<U> | Thenable<U>
  ) => Thenable<U>
}
...
export const thenablify =
  ...
  (...a: A): Thenable<R> => {
    try {
      ...
      return {
        then(f) {
          return thenablify(f, throwImmediately)(r)
        },
        catch() {
          return this as Thenable<never>
        },
      }
    } catch (error) {
      ...
      return {
        then() {
          ...
          return thenablify(f, throwImmediately)(error)
        },
      }
    }
  }
...
const hasThen = (t: unknown): t is { then: unknown } =>
  typeof t === 'object' &&
  t !== null &&
  (t as { then?: never }).then !== undefined
...
export { persist, PersistOptions, PersistentStorage, $$persist, WithPersist }
