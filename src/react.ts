...
type UseStore = <S extends Store<UnknownState>, U = State<S>>(
  store: S,
...
type Create = {
  ...
  <S extends Store<UnknownState>>(store: S): UseBoundStore<S>
}

type UseBoundStore<S> = (<U = State<S>>(
  selector?: (state: State<S>) => U,
  equals?: (a: U, b: U) => boolean
) => U) &
  S
...
type CreateWithState = <T extends UnknownState>() => {
  ...
  <S extends Store<T>>(store: S): UseBoundStore<S>
}
...
const create: Create = (...[storeOrInitializer]: Parameters2<Create>) => {
  ...
  return Object.assign(
    ((selector, equals) => useStore(store, selector, equals)) as UseBoundStore<
      Store<UnknownState>
    >,
    store
  )
