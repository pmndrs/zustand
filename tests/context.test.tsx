...
import create, { createWithState, Store } from 'zustand'
...
it('creates and uses context store', async () => {
  const { Provider, useStore } = createContext<Store<CounterState>>()
...
it('uses context store api', async () => {
  ...
  const { Provider, useStoreRef } = createContext<CustomStore>()

  function Counter() {
    const storeApi = useStoreRef()
...
it('throws error when not using provider', async () => {
  ...
  const { useStore } = createContext<Store<CounterState>>()
  function Component() {
    useStore()
