...
> [!IMPORTANT]
> To use TypeScript we need to install two packages `ts-jest` and `ts-node`.
...
> [!NOTE]
> Without [globals configuration](https://vitest.dev/config/#globals) enabled, we need
...
> [!NOTE]
> Without [globals configuration](https://vitest.dev/config/#globals) enabled, we do
...
> [!NOTE]
> All of these examples are written using TypeScript.
