...
The difference when using TypeScript is that instead of writing `create(...)`, you have to write `create<T>()(...)` (notice the extra parenthesis `()` too along with just the type parameter) where `T` is the type of the state to annotate it. For example:
